...
describe('Auth', () => {
    describe('Register Route', () => {
        ...
        describe('Given an Invalid request payload', () => {
            test('No email: Should return 400-statusCode', async () => {
                const response = await request(app).post('/api/v1/auth/register')
                    .send({
                        password: 'secret'
                    })
            ...
            test('No password: Should return 400-statusCode', async () => {
                const response = await request(app).post('/api/v1/auth/register')
                    .send({
                        email: 'tobemadamori@gmail.com'
                    })
    ...
    describe('Login Route', () => {
        describe('Given the user exists', () => {
            ...
            test.skip('should return 200: User not verified', async () => {
                const response = await request(app).post('/api/v1/auth/login')
                    .send({
                        email: 'tjmadamori1@gmail.com',
                        password: 'secret'
                    })
                expect(response.statusCode).toBe(200)
                expect(response.body.msg).toBe('Incomplete registration error')
                expect(response.body.msg).not.toBe(undefined)
            })
        })
